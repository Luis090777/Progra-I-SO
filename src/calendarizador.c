...
void ordenamiento_costo_bateria_descendente(Accion acciones[], int cantidad_acciones) {
    ...
    for (i = 0; i < cantidad_acciones; i++) {
        for (j = 0; j < cantidad_acciones - i - 1; j++) {
            if (acciones[j].cantidad_de_bateria_requerida < acciones[j+1].cantidad_de_bateria_requerida) {
                printf("%d",acciones[j].cantidad_de_bateria_requerida);
                temp = acciones[j];
                acciones[j] = acciones[j+1];
                acciones[j+1] = temp;
            }
...
void ejecutar_trabajo_costo_bateria_descendente(FILE *log_file, Comuna comuna, Accion acciones[], int cantidad_acciones){
    ...
    ordenamiento_costo_bateria_descendente(acciones, cantidad_acciones);
